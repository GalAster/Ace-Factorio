...
function platformTick()
    if not game.surfaces["AceSpace"] then
        --Create the AceSpace surface as soon as we can. This means we can ensure chunks are generated before we move any entities into space.
        local settings = {
        ...
        game.create_surface("AceSpace", settings)
        game.surfaces["AceSpace"].request_to_generate_chunks({0, 0}, 3)
        game.surfaces["AceSpace"].always_day = true
    end
end

function platformChunkEvent(event)
    --Only handle chunks on our space platform surface.
    if event.surface.name ~= "AceSpace" then return end
    ...
    for x=event.area.left_top.x, event.area.right_bottom.x do
        for y=event.area.left_top.y, event.area.right_bottom.y do
            if x >= 10 or y >= 10 or x < -10 or y < -10 then
                ...
                table.insert(tiles, {name="AceSpace", position={x, y}})
            end
        end
    end
...
function createPlatformEntities()
    ...
    local surface = game.surfaces["AceSpace"]
...
function checkForSpaceOnlyBuildings(event)
    if event.created_entity.surface.name ~= "AceSpace" then
